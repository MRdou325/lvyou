...
<body>
    ...
    <div class="cf">
		<div class="fl">
            <a class="btn" href="<?php echo U('Goods/extraedit',array("goods_id" => $goods_id));?>">新 增</a>
    ...
    <div class="data-table table-striped">
            ...
            <table class="">
                ...
                <tbody>
                <?php if(!empty($extralist)): if(is_array($extralist)): $i = 0; $__LIST__ = $extralist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr class="pointer even" title="">
                        <td><input class="ids" type="checkbox" name="id[]" value="<?php echo ($data["id"]); ?>" /></td>
                        <td><?php echo ($data['name']); ?></a></td>
                        <td><?php echo ($data['sort']); ?></td>
                        <td><a class="confirm ajax-get" href="<?php echo U('Goods/extrastatus',array('id'=>$data['id'],'status'=>$data['status']));?>" title=""><?php echo ($data['status']?'<font color="green">开启</font>':'<font color="red">关闭</font>'); ?></a></td>
                        <td> 
                            <a id="" href="<?php echo U('Goods/extraedit',array('id'=>$data['id']));?>" title="">编辑</a>
                            |
                            <a class="confirm ajax-get" href="<?php echo U('Goods/extradel?id='.$data['id']);?>" title="删除">删除</a>
                        </td>
