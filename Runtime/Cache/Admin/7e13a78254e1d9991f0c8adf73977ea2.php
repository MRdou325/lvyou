                                    ...
                                    <form action="<?php echo U('Index/sortList',array('op'=>'order'));?>" method="post">
                                        <div class="table-responsive">
                                            <table class="table table-striped table-bordered table-hover">
                                                ...
                                                <tbody>
                                                <?php if(!empty($categoryList)): if(is_array($categoryList)): $i = 0; $__LIST__ = $categoryList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr class="pointer even" title="">
                                                        <td><input type="text" name="order_<?php echo ($data['sort_id']); ?>" value="<?php echo ($data['sort']); ?>" style="width:30px;padding:0 5px;margin:0"/></td>
                                                        <td><a href="<?php echo U('CategoryGoods/index',array('id'=>$data['id']));?>"><?php echo ($data['id']); ?>-<?php echo ($data['category_name']); ?></a></td>
                                                        <td class="center"><a href="<?php echo U('CategoryGoods/status',array('id'=>$data['id']));?>"><?php echo ($data['status']==1?'<font color="green">开启</font>':'<font color="red">关闭</font>'); ?></a></td>

                                                        <td>
                                                            <a href="<?php echo U('CategoryGoods/add',array('p_id'=>$data['id']));?>">添加子分类</a>
                                                            |
                                                            <a href="<?php echo U('CategoryGoods/add',array('id'=>$data['id']));?>">编辑</a>
                                                            |
                                                            <a onclick="return confirm('该分类下的商品将全部删除，确定删除')" href="<?php echo U('CategoryGoods/del',array('id'=>$data['id']));?>" title="删除">删除</a>
                                                        </td>
