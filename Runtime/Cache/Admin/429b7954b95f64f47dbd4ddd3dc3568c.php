...
<body>
	...
	<form action="<?php echo U();?>" method="post" class="form-horizontal">
		...
		<div class="form-item">
			<label class="item-label">配置值<span class="check-tips">（配置值）</span> <input type="checkbox" id="ispic"/>图片</label>
			<div class="controls controls-txt">
				<label class="textarea input-large">
					<textarea name="value"><?php echo ((isset($info["value"]) && ($info["value"] !== ""))?($info["value"]):''); ?></textarea>
				</label>
			</div>
			<div class="controls controls-img" style="display:none">
				<label class="item-label"></label>
				<input type="file" id="upload_picture">
				<input type="hidden" name="image" id="image" value=""/>
				<div class="upload-img-box">
				<?php if(!empty($info['value'])): ?><div class="upload-pre-item"><img src="/lvyou<?php echo ((isset($info["value"]) && ($info["value"] !== ""))?($info["value"]):''); ?>"/></div><?php endif; ?>
				</div>
			</div>
			<script type="text/javascript">
				$("#ispic").click(function(){
					if($(this).is(':checked') == true){
						$("div.controls-txt").hide();
						$("div.controls-img").show();
					}
					else{
						$("div.controls-img").hide();
						$("div.controls-txt").show();
					}
				});
				//上传图片
				/* 初始化上传插件 */
				$("#upload_picture").uploadify({
				    ...
				    "buttonText"      : "上传图片",
				    "uploader"        : "<?php echo U('File/uploadWebsite',array('session_id'=>session_id()));?>",
				    "width"           : 120,
				    'removeTimeout'	  : 1,
		...
		<div class="form-item">
			<label class="item-label">配置项<span class="check-tips">（如果是枚举型 需要配置该项）如：0:关闭,1:开启</span></label>
			<div class="controls">
				<label class="textarea input-large">
