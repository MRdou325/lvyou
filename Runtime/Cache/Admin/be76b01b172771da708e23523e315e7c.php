    ...
    <div class="data-table table-striped">
            ...
            <table class="">
                ...
                <?php if(!empty($goodsList)): if(is_array($goodsList)): $i = 0; $__LIST__ = $goodsList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr class="pointer even" title="">
                        ...
                        <td><a href="<?php echo U('extra',array("goods_id" => $data['goods_id']));?>" target='_blank'><?php echo ($data['title']); ?></a></td>
                        <td><?php if($data['goods_type']==1){ ?>普通线路<?php }else{ ?>签证<?php } ?></td>
                        <td><?php echo ($data['price']); ?></td>
						<td><?php echo ($data['discount_price']); ?></td>
                        <td><?php echo ($data['add_uname']); ?></td>
                        <td><a class="confirm ajax-get" href="<?php echo U('Goods/status',array('id'=>$data['goods_id'],'status'=>$data['status']));?>" title=""><?php echo ($data['status']?'<font color="green">开启</font>':'<font color="red">关闭</font>'); ?></a></td>
                        <td><?php echo date('Y-m-d H:i:s',$data['ctime']);?></td>
                        <td> 
                            <a target="_blank" href="<?php echo U('/Home/Goods/info',array('id'=>$data['goods_id']));?>">查看</a>
                            |
                            <a id="" href="<?php echo U('Goods/edit',array('goods_id'=>$data['goods_id']));?>" title="">编辑</a>
                            |
                            <a class="confirm ajax-get" href="<?php echo U('Goods/del?goods_id='.$data['goods_id']);?>" title="删除">删除</a>
                        </td>
