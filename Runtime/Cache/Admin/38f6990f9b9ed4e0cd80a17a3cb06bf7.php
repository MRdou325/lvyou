                                    ...
                                    <form action="<?php echo U('Index/sortList',array('op'=>'order'));?>" method="post">
                                        <div class="table-responsive">
                                            <table class="table table-striped table-bordered table-hover">
                                                ...
                                                <tbody>
                                                <?php if(!empty($TopicModel)): if(is_array($TopicModel)): $i = 0; $__LIST__ = $TopicModel;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr class="pointer even" title="">
                                                        <td><input type="text" name="order_<?php echo ($data['sort_id']); ?>" value="<?php echo ($data['sort']); ?>" style="width:30px;padding:0 5px;margin:0"/></td>
                                                        <td><?php if($data['pic_url'] != ''){ ?><img height="20" src="<?php echo ($data['pic_url']); ?>"><?php } echo ($data['name']); ?></a></td>
                                                        <td class="center"><a href="<?php echo U('Topic/status',array('id'=>$data['tid']));?>"><?php echo ($data['state']==1?'<font color="green">开启</font>':'<font color="red">关闭</font>'); ?></a></td>

                                                        <td>
                                                            <a href="<?php echo U('Topic/add',array('id'=>$data['tid']));?>">编辑</a>
                                                            |
                                                            <a onclick="return confirm('确定删除')" href="<?php echo U('Topic/del',array('id'=>$data['tid']));?>" title="删除">删除</a>
                                                        </td>
