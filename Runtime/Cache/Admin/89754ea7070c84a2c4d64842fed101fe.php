...
<body>
    ...
    <div class="data-table table-striped">
            ...
            <table class="">
                ...
                <tbody>
                <?php if(!empty($goods_type)): if(is_array($goods_type)): $i = 0; $__LIST__ = $goods_type;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr class="pointer even" title="">
                        <td><input class="ids" type="checkbox" name="id[]" value="<?php echo ($data["goods_id"]); ?>" /></td>
                        <td><?php echo ($data['name']); ?></td>
                        <td><?php echo ($data['sort']); ?></td>
                        <td><a class="confirm ajax-get" href="<?php echo U('Goods/goodstypestatus',array('id'=>$data['id'],'status'=>$data['status']));?>" title=""><?php echo ($data['status']?'<font color="green">开启</font>':'<font color="red">关闭</font>'); ?></a></td>
                        <td> 
                            <a id="" href="<?php echo U('Goods/goodstypeedit',array('id'=>$data['id']));?>" title="">编辑</a>
                            |
                            <a class="confirm ajax-get" href="<?php echo U('Goods/goodstypedel', array('id'=>$data['id']));?>" title="删除">删除</a>
                        </td>
