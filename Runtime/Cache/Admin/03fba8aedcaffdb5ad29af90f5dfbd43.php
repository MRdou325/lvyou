		...
		<table>
			...
			<tbody>
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$order): $mod = ($i % 2 );++$i;?><tr>
						...
						<td><a href="<?php echo U('Goods/edit?goods_id='.$order['goods_id']);?>"><?php echo ($order["goods_name"]); ?></a></td>
						<td><del><?php echo ($order["price"]); ?></del><br><?php echo ($order["discount_price"]); ?></td>
						<td><?php echo ($order["num"]); ?>大/<?php echo ($order["child"]); ?>小</td>
