...
<body>
    ...
    <div class="cf">
        ...
        <!-- <div class="search-form fr cf">
            <div class="sleft">
                <input type="text" name="name" class="search-input" value="<?php echo I('name');?>" placeholder="请输入友情链接名称">
                <a class="sch-btn" href="javascript:;" id="search" url="/lvyou/index.php?s=/Admin/FriendLink/index.html"><i class="btn-search"></i></a>
            </div>
        </div> -->
    </div>
                ...
                <form action="<?php echo U('Index/sortList',array('op'=>'order'));?>" method="post">
                    <div class="table-responsive">
                        <table class="table table-striped table-bordered table-hover">
                            ...
                            <tbody>
                                <?php if(!empty($FriendLinkList)): if(is_array($FriendLinkList)): $i = 0; $__LIST__ = $FriendLinkList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr class="pointer even" title="">
                                            <td><input type="text" name="order_<?php echo ($data['sort_id']); ?>" value="<?php echo ($data['sort']); ?>" style="width:30px;padding:0 5px;margin:0"/></td>
                                            <td><?php if($data['pic_url'] != ''){ ?><img height="20" src="<?php echo ($data['pic_url']); ?>"><?php } echo ($data['title']); ?></td>
                                            <td><?php echo ($data['url']); ?></td>
                                            <td class="center"><a href="<?php echo U('FriendLink/status',array('id'=>$data['aid']));?>"><?php echo ($data['state']==1?'<font color="green">开启</font>':'<font color="red">关闭</font>'); ?></a></td>
                                            <td><a href="<?php echo U('FriendLink/add',array('id'=>$data['aid']));?>">编辑</a>
                                                            |
                                                <a onclick="return confirm('该分类下的商品将全部删除，确定删除')" href="<?php echo U('FriendLink/del',array('id'=>$data['aid']));?>" title="删除">删除</a>
                                            </td>
