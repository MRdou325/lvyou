...
function is_login($admin = FALSE){
    ...
    if (empty($user)) {
        ...
        if($admin){
            return session('user_auth_sign_admin') == data_auth_sign($user) ? $user['uid'] : 0;
        }
...
function is_administrator($uid = null){
    $uid = is_null($uid) ? is_login(true) : $uid;
...
function get_goodstype_name($id){
    $type = M('goods_type')->find($id);
    return $type["name"];
}